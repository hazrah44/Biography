...
async function getDegrees(url) {
  ...
  await fetch(url)
    ...
    .then((data) =>
      document.write(
        `My first degree was a ${data.data[0].degrees.bachelors.data.type} from ${data.data[0].degrees.bachelors.data.institution} in ${data.data[0].degrees.bachelors.data.date} and master ${data.data[0].degrees.bachelors.data.futuregraduation}`
      )
    );
}
